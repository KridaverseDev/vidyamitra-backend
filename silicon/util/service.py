...
from .discovery import get_dev_port
...
class Service:
    def __init__(self, name_or_file: str):
        if os.path.sep in name_or_file:
            ...
            self._name = self._resolve_service(rparts)
        else:
            self._name = name_or_file

    @staticmethod
    def _resolve_service(rparts):
        return ".".join(reversed(rparts[1 : rparts.index("silicon") + 1]))

    @staticmethod
    def _resolve_serive_name(name):
        service_path = name.split(".")
        return f"{service_path[0]}-{service_path[2]}"

    def hello(self, settings):
        from pyfiglet import Figlet
        from termcolor import colored

        service = self._resolve_serive_name(self._name)
    ...
    def wsgi_app(self):
        ...
        from silicon.wsgi import application as app
    ...
    def run_gunicorn(self):
        ...
        if len(sys.argv) == 1:
            sys.argv.extend(
                ["--config", "python:silicon.gunicorn_conf", "silicon.wsgi"]
            )
        os.environ.setdefault("DJANGO_SETTINGS_MODULE", f"{self._name}.settings")
